...
function prepareQueue({
  ...
  const {
    ...
    height,
    aspectRatio
  } = calculateImageDimensionsAndAspectRatio(file, args);
...
function lazyJobsEnabled() {
  return process.env.gatsby_executing_command === `develop` && (!isCI() || process.env.GATSBY_ENABLE_LAZY_IMAGES_IN_CI) && !(process.env.ENABLE_GATSBY_EXTERNAL_JOBS === `true` || process.env.ENABLE_GATSBY_EXTERNAL_JOBS === `1`);
}
...
async function fluid({
  ...
  let filteredSizes = fluidSizes.filter(size => size < (fixedDimension === `maxWidth` ? width : height));
  ...
  const imageWithDensityOneIndex = filteredSizes.findIndex(size => size === (fixedDimension === `maxWidth` ? maxWidth : maxHeight));
